...
export function SeatGrid({ seats }: { seats: string[] }): ReactElement {
  ...
  return (
    <section aria-labelledby="seat-grid-title">
      <h2 id="seat-grid-title">Your seats</h2>
      <div className="seat-grid" aria-label="Static seat grid">
        {labels.map((seat) => (
          <span
            className={seats.includes(seat) ? 'seat selected-seat' : 'seat'}
            key={seat}
            aria-label={`${seat}${seats.includes(seat) ? ', selected' : ', unavailable'}`}
          >
            {seat}
          </span>
        ))}
