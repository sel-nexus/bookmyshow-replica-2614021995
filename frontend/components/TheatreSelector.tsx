...
export function TheatreSelector({ theatres, selectedTheatreId, onSelect }: TheatreSelectorProps): ReactElement {
  return <section aria-labelledby="theatres-heading"><h2 id="theatres-heading">Choose a theatre</h2><div role="list" aria-label="Theatres">{theatres.map((theatre) => <button key={theatre.id} type="button" aria-pressed={selectedTheatreId === theatre.id} onClick={() => onSelect(theatre)}>{theatre.name}</button>)}</div></section>;
}
